...
export function validateItem(value: unknown): ItemInstance | undefined {
  ...
  if (item.kind === 'armor') item.kind = 'chest';
  ...
  for (const key of ['cooldownReduction', 'movementSpeed', 'manaRegen'] as const) {
    const v = input[key]; stats[key] = typeof v === 'number' && Number.isFinite(v) ? Math.max(0, Math.min(key === 'movementSpeed' ? .1 : key === 'manaRegen' ? 3 : .2, v)) : 0;
  }
