...
export class EmberSpider {
  ...
  public constructor(private readonly scene: Phaser.Scene, x: number, y: number, _player: PlayerCharacter, private readonly onDeath: (spider: EmberSpider, x: number, y: number) => void, private readonly onEngage: (spider: EmberSpider) => void = () => undefined, public readonly elite?: EliteAffix, health = 58) {
  ...
  public update(time: number): void {
    ...
    if (!target) { this.visual.setVelocity(0); return; }
    const distance = Phaser.Math.Distance.Between(this.visual.x, this.visual.y, target.x, target.y);
    if (distance < 43 && time >= this.nextAttackAt) { this.attackTargetId = target.targetId; this.state = 'attack'; this.impact = false; this.visual.setVelocity(0).play(EmberSpiderAnimation.Attack, true); return; }
    if (distance < 210) { this.state = 'chase'; this.scene.physics.moveToObject(this.visual, target.physicsRoot, 57 * this.modifiers.speedMultiplier); this.visual.setFlipX(target.x < this.visual.x).play(EmberSpiderAnimation.Move, true); }
  ...
  private onFrame(animation: Phaser.Animations.Animation, frame: Phaser.Animations.AnimationFrame): void { if (this.modifiers.stunned || this.state !== 'attack' || this.impact || animation.key !== EmberSpiderAnimation.Attack || Number(frame.textureFrame) !== 2) return; this.impact = true; const target = combatTargets(this.scene).get(this.attackTargetId); if (target && Phaser.Math.Distance.Between(this.visual.x, this.visual.y, target.x, target.y) <= 50 && target.takeDamage(Math.round(14 * this.modifiers.damageMultiplier), this.visual.x, this.visual.y)) this.onEngage(this); }
